...
###########Meghana's Model Agnostic Interpretability on Naive Bayes#############3
# NB
# tuning parameters
tune_grid <- expand.grid(
  usekernel = c(TRUE, FALSE),
...
training.df$pred_rf <- predict(nb.bank)
...
# ICE for naive bayes
set.seed(12345)
nb_pred <- Predictor$new(nb.bank, data = training[,-1], 
                             y = training$Sale_Price, type = "response")

ice_plot <- FeatureEffects$new(nb_pred, method = "ice")
ice_plot$plot(c("Garage_Area"))
ice_plot$plot()

# PDP for naive bayes
pd_plot <- FeatureEffects$new(nb_pred, method = "pdp")
pd_plot$plot(c("Garage_Area"))
pd_plot$plot()

# PDP on ICE for naive bayes
pdice_plot <- FeatureEffects$new(nb_pred, method = "pdp+ice")
...
# ALE for naive bayes
ale_plot <- FeatureEffects$new(nb_pred, method = "ale")
ale_plot$plot(c("Garage_Area"))
ale_plot$plot()

# LIME for naive bayes
point <- 732
lime.explain <- LocalModel$new(nb_pred, x.interest = training[point,-1], k = 5)
...
# Shapley Values for naive bayes
point <- 732
shap <- Shapley$new(nb_pred, x.interest = training[point,-1])
shap$plot()
